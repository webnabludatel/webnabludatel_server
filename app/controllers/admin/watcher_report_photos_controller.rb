class Admin::WatcherReportPhotosController < Admin::BaseController
  ...
  def index
    @watcher_report_photos = WatcherReportPhoto.includes(:watcher_report => :user).order("id desc").page(params[:page])
  end
  ...
  def update
    ...
    if @watcher_report_photo.update_attributes(:status => params[:status])
      ...
      render :js => "$('##{dom_id(@watcher_report_photo, :controls)}').append('<p>ERROR: #{@watcher_report_photo.errors.full_messages.inspect}');"
    end
  end
end